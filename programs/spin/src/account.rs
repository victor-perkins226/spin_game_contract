...
impl SpinItemList {
    ...
    pub fn get_spinresult(&mut self, rand: u32) {
        ...
        let mut start = 0;
        let unaligned = std::ptr::addr_of!(self.ratio_list);
        let temp_ratio_list = unsafe { std::ptr::read_unaligned(unaligned) };
        for (pos, item) in temp_ratio_list.iter().enumerate() {
            let end = start + item;
